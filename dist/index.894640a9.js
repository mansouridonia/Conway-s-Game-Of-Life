...
})({"aWbRx":[function(require,module,exports) {
"use strict";
...
if (!!canavs) {
    ...
    if (!!ctx) {
        ...
        ctx.lineWidth = 1;
        const drawBorder = ()=>{
            for(let i3 = 0; i3 < TILES_X; i3++){
                ctx.beginPath();
                ctx.moveTo(i3 * TILE_SIZE - 0.5, 0);
                ctx.lineTo(i3 * TILE_SIZE - 0.5, height);
                ctx.stroke();
            }
        ...
        board[3][1] = true;
        drawAndClear();
        const game = new (0, _recursive.GameOfLife)(board, TILE_SIZE, ctx);
        ctx.strokeStyle = "rgb(9, 90, 90)";
        for(let i1 = 0; i1 <= width; i1++){
            for(let j = 0; j <= height; j++)if (board[i1][j]) ctx.fillRect(i1 * TILE_SIZE, j * TILE_SIZE, TILE_SIZE, TILE_SIZE);
        }
    // game.applyRule();
    //for (let i = 0; i < 1; i++)
    //game.applyRule();
...
},{"./recursive":"20nS0"}],"20nS0":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "GameOfLife", ()=>GameOfLife) /*const game = new GameOfLife();
for (let i = 0; i < 10; i++) game.applyRule();*/  /*if (!this.allAreFalse()) */ ;
class Node {
...
class GameOfLife {
    cels = [];
    constructor(config, TILE_SIZE, ctx){
        this.cels = config;
    ...
    display() {
        this.ctx.strokeStyle = "rgb(9, 90, 90)";
        //this.ctx.clearRect(0, 0, this.width, this.heigh);
        for(let i = 0; i <= this.width; i++){
            for(let j = 0; j <= this.heigh; j++)if (this.cels[i][j]) this.ctx.fillRect(i * this.TILE_SIZE, j * this.TILE_SIZE, this.TILE_SIZE, this.TILE_SIZE);
        }
    }
    applyRule = ()=>{
        let finalCells = this.cels.map((arr)=>arr.slice());
        for(let _i = 0; _i <= this.width; _i++)for(let _j = 0; _j <= this.heigh; _j++){
    ...
    /******** Get Neighbours' count of a Node *****************/ getCptN = (neighbours)=>{
        let s = 0;
    ...
    /******** Get Neighbours of a Node *****************/ getNeighbours = (i, j)=>{
        ...
        if (j < this.heigh) myNeighbours.push(new Node(i, j + 1));
        return myNeighbours;
    };
}
...
},{}]},["aWbRx","7wTDB"], "7wTDB", "parcelRequire94c2")

//# sourceMappingURL=index.894640a9.js.map
